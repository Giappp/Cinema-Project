(() => {
	...
	const btnClose = $('#myModal2 .btnClose');
	...
	const listGift = [
        ...
        {
			text: 'Bến phà xác sống',
			percent: 16.5 / 100,
		},
		{
			text: 'Siêu lừa gặp siêu lầy',
			percent: 16.5 / 100,
		},
	];
	...
	listGift.map((item, index) => {
		...
		switch (index) {
			case 0:
				elm.innerHTML = `<p style="transform: skewY(${skewY}deg) 
					rotate(${rotate / 2}deg); " class="text text-1">
					<b>${item.text}</b>
				</p>`;
				break;
			case 1:
				elm.innerHTML = `<p style="transform: skewY(${skewY}deg) 
					rotate(${rotate / 2}deg); " class="text text-2">
					<b>${item.text}</b>
				</p>`;
				break;
			case 2:
				elm.innerHTML = `<p style="transform: skewY(${skewY}deg) 
					rotate(${rotate / 2}deg); " class="text text-3">
					<b>${item.text}</b>
				</p>`;
				break;
			case 3:
				elm.innerHTML = `<p style="transform: skewY(${skewY}deg) 
					rotate(${rotate / 2}deg); " class="text text-4">
					<b>${item.text}</b>
				</p>`;
				break;
			case 4:
				elm.innerHTML = `<p style="transform: skewY(${skewY}deg) 
					rotate(${rotate / 2}deg); " class="text text-5">
					<b>${item.text}</b>
				</p>`;
				break;
			case 5:
				elm.innerHTML = `<p style="transform: skewY(${skewY}deg) 
					rotate(${rotate / 2}deg); " class="text text-6">
					<b>${item.text}</b>
				</p>`;
				break;
		}
	...
	const showGift = gift => {
		let timer = setTimeout(() => {
			isRotating = false;
			btnOn();
			showMsg1.innerHTML = "Vũ trụ chọn cho bạn"
